...
export const HistoryScreen = ({ onBack, userId, trainerId }: HistoryScreenProps) => {
                ...
                <div key={item.id} className="bg-zinc-900 rounded-2xl border border-zinc-800 overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {item.workoutDetails?.exercises?.length > 0 && (
                  ...
                  {expandedId === item.id && item.workoutDetails?.exercises?.length > 0 && (
                    <div className="px-4 pb-4 border-t border-zinc-800 pt-3 animate-in fade-in slide-in-from-top-2 duration-200">
                      <h4 className="text-[10px] uppercase tracking-widest text-[#D4AF37] mb-2 font-bold">Detalles del entrenamiento</h4>
                      <div className="space-y-2">
                        {item.workoutDetails.exercises.map((ex: any, idx: number) => (
                          <div key={idx} className="flex justify-between text-sm py-1 border-b border-zinc-800/50 last:border-0">
                            <span className="text-zinc-300">{ex.name}</span>
                            <span className="text-zinc-500">{ex.sets}x{ex.reps} {ex.weight ? `@ ${ex.weight}` : ''}</span>
                          </div>
                        ))}
                      </div>
                    </div>

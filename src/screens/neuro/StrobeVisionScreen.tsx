...
export const StrobeVisionScreen: React.FC = () => {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      console.warn('Strobe Camera access error:', err);
    }
  }, [cameraFacing]);
  ...
  // Strobe Shutter Cycle Loop
  const runStrobeCycle = useCallback(() => {
    setIsOccluded(false);
    playStrobeClick('open');

    strobeTimerRef.current = setTimeout(() => {
      setIsOccluded(true);
      playStrobeClick('close');
  ...
  const animateBall = useCallback(() => {
    ...
    const progress = Math.min(100, (elapsed / ballDurationMsRef.current) * 100);
  ...
  // Handle athlete intercepting ball in anticipation mode
  const handleInterceptTap = () => {
    if (mode !== 'BALL_ANTICIPATION' || !isDrillActive) return;

    const targetProgress = 75; // Intercept line at 75%
    const currentProg = ballProgress;
    const errorPct = currentProg - targetProgress;
    const errorMs = Math.round((errorPct / 100) * ballDurationMsRef.current);

    playStrobeClick('hit');

    let rating: 'perfect' | 'good' | 'miss' = 'miss';
    let text = 'Fuera de Tiempo';

    if (Math.abs(errorMs) <= 65) {
      rating = 'perfect';
      text = `¡Excelente! (${errorMs > 0 ? '+' : ''}${errorMs}ms)`;
    } else if (Math.abs(errorMs) <= 150) {
      rating = 'good';
      text = `Buen Intento (${errorMs > 0 ? '+' : ''}${errorMs}ms)`;
    } else {
      rating = 'miss';
      text = errorMs < 0 ? 'Muy Temprano' : 'Muy Tarde';
    }

    setAnticipationFeedback({ errorMs, text, type: rating });
    setSessionResults(prev => [...prev, { rep: prev.length + 1, errorMs: Math.abs(errorMs), rating }]);
  };
  ...
  const handleSaveToFirestore = async () => {
    if (!user?.uid || elapsedSec === 0 || isSaving) return;
    setIsSaving(true);
    ...
    try {
      ...
      await addDoc(collection(db, 'reactionTests'), {
        userId: user.uid,
        type: 'STROBE_OCCLUSION_VISION',
        title: 'Entrenamiento Visual Estroboscópico',
        date: new Date().toISOString(),
        mode,
        durationSec: elapsedSec,
        level: isCustomPreset ? 0 : STROBE_PRESETS[selectedLevelIndex].level,
        visibleMs: currentVisibleMs,
        occludedMs: currentOccludedMs,
        avgAnticipationErrorMs: avgError,
      ...
      console.error('Error saving Strobe result:', err);
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <header className="p-3 sm:p-5 border-b border-zinc-900 bg-zinc-950/90 backdrop-blur-md flex items-center justify-between z-30 sticky top-0">
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div>
            ...
            <h1 className="text-sm sm:text-lg font-black text-white leading-tight">Entrenamiento de Oclusión Estroboscópica</h1>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-3 sm:p-6 relative overflow-hidden">
        {!isDrillActive && !isFinished ? (
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-black text-white">Metodología de Información Reducida</h2>
                  <p className="text-xs text-zinc-400">Emula las gafas estroboscópicas Senaptec utilizadas en la NBA y fútbol de élite.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-5 gap-1.5 sm:gap-2 mb-3">
                {STROBE_PRESETS.map((lvl, idx) => (
                  <button
                    ...
                    className={`py-2.5 rounded-2xl font-bold text-xs transition-all border flex flex-col items-center justify-center gap-0.5 ${
                  ...
                  >
                    <span className="text-xs">L{lvl.level}</span>
          ...
          <div className="w-full max-w-4xl flex flex-col items-center flex-1 justify-center">
            ...
            <div className="mt-4 flex items-center gap-3 w-full max-w-sm justify-center">
              {mode === 'BALL_ANTICIPATION' && (
                <button
                  onClick={handleInterceptTap}
                  className="flex-1 py-3.5 bg-[#D4AF37] hover:bg-[#e6c158] text-black font-black text-xs uppercase tracking-wider rounded-2xl flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(212,175,55,0.4)] active:scale-95"
                >
                  ⚡ Interceptar Ahora
                </button>
              )}
              ...
              <button
                onClick={handleToggleDrill}
                className="flex-1 py-3.5 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/40 font-bold text-xs uppercase tracking-wider rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95"
              >
          ...
          >
            ...
            <h2 className="text-xl sm:text-2xl font-black text-white">Sesión Estroboscópica Finalizada</h2>
            ...
            <div className="grid grid-cols-3 gap-2.5 sm:gap-3 my-6">
              <div className="bg-black/60 border border-zinc-800 p-3.5 rounded-2xl">
                ...
                <p className="text-xl sm:text-2xl font-mono font-black text-[#D4AF37]">{elapsedSec}s</p>
              ...
              <div className="bg-black/60 border border-zinc-800 p-3.5 rounded-2xl">
                ...
                <p className="text-xl sm:text-2xl font-mono font-black text-cyan-400">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                <RotateCcw size={16} /> Repetir Ejercicio
              </button>
